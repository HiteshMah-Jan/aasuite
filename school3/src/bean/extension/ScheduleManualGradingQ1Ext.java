...
@ChildRecords({
    @ChildRecord(entity = StudentSubjectManualGradingQ1Ext.class, template=ChildTemplateListOnly.class, title="Q1", sql = "SELECT a FROM StudentSubject a, Student b WHERE a.studentId=b.personId AND a.scheduleId=${seq} AND a.schoolYear='${useYear}' ORDER BY b.gender DESC, b.lastName", fieldMapping = {"seq", "scheduleId"}),

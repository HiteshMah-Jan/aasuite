 ...
 */
@UITemplate(template=template.screen.TemplateTabSinglePage.class, gridCount=4, rule="PaymentEnrollment", title="Accounts Receivable", 
    columnSearch = {"cashier","paidBy", "dueDate", "amount", "paid", "paymentDate", "forApproval1", "forApproval2", "approvalStatus1", "approvalStatus2"}, 
    criteriaSearch = {"cashier","paidBy", "dueDate", "amount", "paid", "paymentDate"}, 
    select="SELECT a FROM PaymentEnrollment a WHERE a.paid=false ", orderBy="a.paymentDate", autoResizeTable=false)
@Displays({
        ...
        @Display(name="surcharge", width=-1),
...
public class PaymentAllNotPaidExt extends bean.sales.Payment implements Serializable {

...
@UITemplate(template=template.screen.TemplateTabSinglePage.class,
    columnSearch={"schoolYear", "faculty", "section", "subject", "component"}, autoResizeTable=false,
    criteriaSearch={"schoolYear", "facultyId", "section", "subject"},
    gridCount=6, title="Component Grade Posting", select="SELECT a FROM FacultyGradingTask a WHERE a.facultyId=${personId}")
@ChildRecords(value={
    @ChildRecord(entity=Q4StudentSubjectComponentScoreExt.class, template=ChildTemplateListPopupDownButton.class, fieldMapping={"seq","facultyGradingTaskId"}, sql="SELECT a FROM StudentSubjectDetailGrading a WHERE a.facultyGradingTaskId='${seq}' ORDER BY a.studentName", title="Raw Scores"),
    @ChildRecord(entity=QAllStudentSubjectComponentScoreCollegeExt.class, template=ChildTemplateListPopupDownButton.class, fieldMapping={"seq","facultyGradingTaskId"}, sql="SELECT a FROM StudentSubjectDetailGrading a WHERE a.facultyGradingTaskId='${seq}' ORDER BY a.studentName", title="Student Score Summary")
})

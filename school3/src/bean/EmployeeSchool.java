...
@Entity
@Table(name = "Person")
@UITemplate(canBackup=false, template = TemplateSinglePage.class, gridCount = 6, columnSearch = {"lastName", "firstName","department"},showImages=true)
@ChildRecords(
    value={
       ...
       @ChildRecord(title="Deduction", entity = EmployeeDeduction.class, sql = "SELECT a FROM EmployeeDeduction WHERE a.personId = ${personId}", fieldMapping={"personId","personId"}),
...
@DiscriminatorValue("SCHOOLEMPLOYEE")
@Displays({
...
public class EmployeeSchool extends Employee implements Serializable {  
}

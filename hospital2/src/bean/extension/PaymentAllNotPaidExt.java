 ...
 */
@UITemplate(template=template.screen.TemplateTabPage.class, gridCount=8, showChart=true, title="Accounts Receivable", 
    ...
    select="SELECT a FROM Payment a WHERE a.paid=false ", orderBy="a.paymentDate")
@Displays({
        @Display(name="paidBy", label="Student", type="PopSearch", gridFieldWidth=3, linktoBean=Patient.class, width=-1),

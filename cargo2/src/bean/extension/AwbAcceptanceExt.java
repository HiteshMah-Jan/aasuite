...
@Reports({
    ...
    @template.Report(reportFile="AwbCheckin", reportTitle="Checkin", reportSql = "${seq}"),
...
@ChildRecords(
    value={
        @ChildRecord(template=ChildTemplateListPopupDownButton.class, entity=AwbCheckin.class, sql="SELECT a FROM AwbCheckin a WHERE a.awbSeq='${seq}'", title="Checkin", fieldMapping={"seq","awbSeq"}),
...
@template.ActionButtons({
    @template.ActionButton(name="btnAssignToULD", label="Assign To ULD"),
    @template.ActionButton(name="btnAssignToFlight", label="Assign To Flight"),
    @template.ActionButton(name="btnNotoc", label="Notoc"),
    @template.ActionButton(name="btnShowMessages", label="Show Messages")
})
public class AwbAcceptanceExt extends bean.Awb {

...
@Reports({
    ...
    @template.Report(reportFile="AwbDeliver", reportTitle="Delivery", reportSql = "${seq}"),
...
@ChildRecords(
    value={
        @ChildRecord(template=ChildTemplateListPopupDownButton.class, entity=AwbDeliver.class, sql="SELECT a FROM AwbDeliver a WHERE a.masterAwbSeq='${seq}'", title="Arrival/Deliver", fieldMapping={"seq","masterAwbSeq"}),
...
public class AwbDeliveryExt extends bean.Awb {

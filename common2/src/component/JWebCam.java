...
public class JWebCam extends JFrame implements WindowListener, ComponentListener {
    ...
    protected static final int MIN_WIDTH = 100; // 320;
    protected static final int MIN_HEIGHT = 100; // 240;
    ...
    public JPanel getVisualContainer() {
...
//                Log.out("Note : " + cams.length + " web cams detected");
    ...
    protected void toolbarHandler(MyToolBarAction actionBtn) {
        if (actionBtn == formatButton) {
            Object selected = JOptionPane.showInputDialog(this, "Select Video format", "Capture format selection", JOptionPane.INFORMATION_MESSAGE, null, myFormatList, currentFormat);
            if (selected != null) {
    ...
    public MyCaptureDeviceInfo[] autoDetect() {
        ...
        if (list != null) {
            ...
            if (capDevices.isEmpty()) {
                capDevices.addElement(new MyCaptureDeviceInfo(new CaptureDeviceInfo("vfw://0",new MediaLocator("vfw:Microsoft WDM Image Capture (Win32):0"),null)));
                capDevices.addElement(new MyCaptureDeviceInfo(new CaptureDeviceInfo("vfw://1",new MediaLocator("vfw:Microsoft WDM Image Capture (Win32):1"),null)));
                capDevices.addElement(new MyCaptureDeviceInfo(new CaptureDeviceInfo("vfw://2",new MediaLocator("vfw:Microsoft WDM Image Capture (Win32):2"),null)));
            }
        }

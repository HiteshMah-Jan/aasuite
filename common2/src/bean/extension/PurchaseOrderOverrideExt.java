...
@UITemplate(template = TemplateTabSinglePage.class, gridCount = 4, columnSearch = {
		"purchaseDate", "purchaseFrom", "totalPOAmount" }, select="SELECT a FROM PurchaseOrder a WHERE a.approvedBy IS NOT NULL AND a.approvedBy=a.approvedBy2")
...
@Displays( {
		...
		@Display(name = "invoiceNumber", width = -1, enabled=false),
		@Display(name = "invoiceDate", enabled=false),
...
public class PurchaseOrderOverrideExt extends PurchaseOrder {
	public static void main(String[] args) {
		view(PurchaseOrderOverrideExt.class);
	}
}

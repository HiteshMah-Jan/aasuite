 ...
 * @author Entokwaa
 */
@UITemplate(showChart=false, template=TemplateTabSinglePage.class, title="Follow Up",
    ...
    select="SELECT a FROM CustomerContact a WHERE a.customerType='FOLLOW UP'", autoResizeTable=false)
@Displays({
...
@ActionButtons({
    ...
    @ActionButton(name="btnTodayCallFollowup", label="Display Today's Follow Up"),
    @ActionButton(name="btnWeekCallFollowup", label="Display This Week Follow Up"),
    @ActionButton(name="btnPresentation", label="For Presentation"),
    @ActionButton(name="btnSales", label="For Sales"),
    @ActionButton(name="btnNegative", label="Negative")
})
public class CustomerContactFollowupExt extends CustomerContact {
    public static void main(String[] args) {
        view(CustomerContactFollowupExt.class);
    }
    ...
    public java.util.Vector allChart() {
        ...
        vec.add(ChartBean.getPieInstance(this, location+" By Class","SELECT a.customerClass, COUNT(a.personId) FROM CustomerContact a WHERE a.customerType='FOLLOW UP' AND a.location='"+location+"' GROUP BY a.customerClass"));
        vec.add(ChartBean.getPieInstance(this, "Follow Up By Call Date","SELECT a.nextCallDate, COUNT(a.personId) FROM CustomerContact a WHERE a.customerType='FOLLOW UP' GROUP BY a.nextCallDate"));
        return vec;
    }   
}

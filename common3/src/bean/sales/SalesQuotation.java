...
@Displays({
    	@Display(name="dummyField", type="MergePanel", noLabel=true, fieldPrefix="logistics.", 
    			mergeFields={"customerId","contactPerson","customerRefNo","localCurrency","salesEmployeeId","owner","itemOrServiceType"}, verticalMerge=true),
    	...
    	@Display(name="dummyField", type="MergePanel", noLabel=true, fieldPrefix="logistics.", 
    			mergeFields={"status","postingDate","deliveryDate","documentDate","remarks"}, verticalMerge=true),
        @Display(name="status"),
...
@ChildRecords(
		value={
				@ChildRecord(template=ChildTemplateListPopupDownButton.class,entity=SalesQuotationItem.class, fieldMapping={"seq","salesQuotationId"}, sql="SELECT a FROM SalesQuotationItem a WHERE a.salesQuotationId=${seq}", title="Contents")
		},
		info={
...
public class SalesQuotation extends AbstractIBean implements Serializable {
	public static void main(String[] args) {
		view(SalesQuotation.class);
	}
	
	@Override
    public void save() {
		this.logistics = ((SalesQuotation)myClone).logistics;
		this.accounting = ((SalesQuotation)myClone).accounting;
        super.save();
    }
